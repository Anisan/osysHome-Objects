...
## 5. Тип свойства color

Для работы с цветом используйте тип `color`. Подробное руководство: `docs/COLOR_TYPE_USAGE.ru.md`.
...
- Яркость храните отдельно (`brightness`).
- Для Zigbee2MQTT используйте `write_format=auto`.
- Поле `params.color` (в Additional Parameters) остается цветом иконки свойства и не относится к типу `color`.
...
- **Полная документация:** `docs/PARAMS_DOCUMENTATION.md`
